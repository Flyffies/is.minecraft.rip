...
$m->set('mc_status', json_encode($Answer), 1800) or die('Couldnt save');
...
if (($BackOnline = $m->get('mc_status_back_online'))) {
    ...
    foreach ($BackOnline as $Service => $Downs) {
        if ($Raw[$Service] === MinecraftStatusChecker::STATUS_OFFLINE) {
            $m->set('mc_status_' . $Service, ++$Downs, 120) or die('Could not save!');
        } else {
...
if (!Empty($BackOnline)) {
    $m->set('mc_status_back_online', $BackOnline, 120) or die('Could not save!');
}
...
function HandleNews($Data, $Code)
{
    ...
    if ($Data === false || Empty($Data)) {
        $m->set('mc_status_mojang', '', 300) or die('Could not save!');
    ...
    $m->set('mc_status_mojang', $PSA, 300) or die('Could not save');
}

$json = fopen('../status.json', 'w');
fwrite($json, ($m->get('mc_status')));
fclose($json);
